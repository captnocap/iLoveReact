...
| `tools/seat selection` | `{"action":"selection"}` | The LIVE selection, measured (req_4202). Mode, count, pivot/bounds, and one row per selected vertex/edge/triangle — an edge row carries its endpoints, length, part, and how many faces it ALREADY carries (`faces`/`open`). Also carries `createFace`: Create Face's own gate ladder read against this selection, split into `blocking` (measured — the shape is wrong) and `hostDecides` (scope and winding, which live in host state this snapshot cannot see, and are never reported as passing). Read-only: it sets no selection, changes no mode, touches no geometry. Reach for it whenever a topology verb refuses — the alternative is pressing the key again, which mutates on the runs where it works. `elements` describes the MESH; this describes what is selected in it. |
