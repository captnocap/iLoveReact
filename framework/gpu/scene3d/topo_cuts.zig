...
/// Convert every selected edge into a square-section strut in the same Outliner
/// part. Complete selected face boundaries are consumed by the indexed operation;
/// partial selections preserve their unrelated surface. The entire network lands
/// as one undo/journal mutation.
pub fn meshTopoEdgeTubes(radius_raw: f32) bool {
    z3d.g_topo_refusal = "";
    if (z3d.g_lc != null or z3d.g_bevel != null or z3d.g_quadify != null or !model_paint.hasTarget()) return false;
    if (mesh_edit.mode() != .edge) {
        z3d.topoRefuse("Edge to Tubes works in Edge select mode");
        return false;
    }
    const selected_count = mesh_edit.selectedEdgeCountPub();
    if (selected_count == 0) {
        z3d.topoRefuse("select one or more edges to turn into tubes");
        return false;
    }
    const selected = std.heap.c_allocator.alloc(mesh_edit.Edge, selected_count) catch return false;
    defer std.heap.c_allocator.free(selected);
    if (mesh_edit.selectedEdgesPub(selected) != selected_count) return false;
    const selected_part = mesh_edit.selectedEdgesCommonPartPub() orelse {
        z3d.topoRefuse("Edge to Tubes needs selected edges from one Outliner part");
        return false;
    };
    const verts = z3d.g_edit_verts orelse return false;
    const tri_count = z3d.g_edit_count / 3;
    ...
    // Mirror Edit extends the source network atomically and deduplicates edges
    // lying on an enabled symmetry plane.
    if (mesh_edit.mirrorMask() != 0) {
        for (selected) |source_edge| {
            var subset: u8 = 1;
            while (subset <= 7) : (subset += 1) {
    ...
    const radius = if (std.math.isFinite(radius_raw) and radius_raw >= indexed_edit_mesh.EdgeTubeTuning.minimum_radius_m)
        radius_raw
    else
        indexed_edit_mesh.EdgeTubeTuning.default_radius_m;
    if (!(indexed.edgeTubes(resolved.items, radius) catch return false)) {
        z3d.topoRefuse("the selected network is too short for a durable tube at that width");
        return false;
    }
    var lowered = indexed.lower() catch return false;
    defer lowered.deinit();
    ...
    var snap = z3d.journalSnapshotCurrent("edge to tubes");
    const install_groups: ?[]const u32 = if (groups_arg != null) lowered.groups else null;
    const installed = z3d.lcInstallLowered(
        lowered.positions,
        lowered.uvs,
