...
pub fn registerQjsHostFunctions() void {
    ...
    reg("__audio_set_tempo", @ptrCast(&hostAudioSetTempo), 2);
    reg("__audio_make_beat", @ptrCast(&hostAudioMakeBeat), 4);
    reg("__audio_make_beat_slice", @ptrCast(&hostAudioMakeBeatSlice), 5);
    reg("__audio_insert_media", @ptrCast(&hostAudioInsertMedia), 3);
    reg("__audio_fit_media", @ptrCast(&hostAudioFitMedia), 4);
    reg("__audio_insert_media_section", @ptrCast(&hostAudioInsertMediaSection), 5);
    reg("__audio_clear_track", @ptrCast(&hostAudioClearTrack), 1);
    reg("__audio_set_track_volume", @ptrCast(&hostAudioSetTrackVolume), 2);
    reg("__audio_set_track_pan", @ptrCast(&hostAudioSetTrackPan), 2);
    reg("__audio_set_track_mute", @ptrCast(&hostAudioSetTrackMute), 2);
    reg("__audio_set_track_solo", @ptrCast(&hostAudioSetTrackSolo), 2);
    reg("__audio_set_step_velocity", @ptrCast(&hostAudioSetStepVelocity), 3);
    reg("__audio_set_step_probability", @ptrCast(&hostAudioSetStepProbability), 3);
    reg("__audio_set_step_offset", @ptrCast(&hostAudioSetStepOffset), 3);
    reg("__audio_dur", @ptrCast(&hostAudioDur), 1);
    reg("__audio_create_audio_stretch", @ptrCast(&hostAudioCreateAudioStretch), 2);
    reg("__audio_create_audio_slice", @ptrCast(&hostAudioCreateAudioSlice), 3);
    reg("__audio_play", @ptrCast(&hostAudioPlay), 0);
    reg("__audio_transport_pause", @ptrCast(&hostAudioTransportPause), 0);
    reg("__audio_stop", @ptrCast(&hostAudioStop), 0);
    reg("__audio_set_playhead", @ptrCast(&hostAudioSetPlayhead), 1);
    reg("__audio_get_playhead", @ptrCast(&hostAudioGetPlayhead), 0);
    reg("__audio_is_playing", @ptrCast(&hostAudioIsPlaying), 0);
