...
fn argStr(alloc: std.mem.Allocator, info: v8.FunctionCallbackInfo, idx: u32) ?[]u8 {
...
fn hexEncodeAlloc(alloc: std.mem.Allocator, bytes: []const u8) ![]u8 {
    const out = try alloc.alloc(u8, bytes.len * 2);
    crmod.bytesToHex(bytes, out);
    return out;
}

fn hexDecodeAlloc(alloc: std.mem.Allocator, hex: []const u8) ![]u8 {
...
fn privSha256(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const alloc = host.gpa;
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const data_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(data_b64);

    const data = b64Decode(alloc, data_b64) catch return returnEmpty(info);
    defer alloc.free(data);

    const digest = privacy.sha256With(parseBackend(backend_s), data);
    const enc = b64Encode(alloc, &digest) catch return returnEmpty(info);
...
fn privHmacSha256(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const key_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(key_b64);
    const msg_b64 = argStr(alloc, info, 2) orelse return returnEmpty(info);
    defer alloc.free(msg_b64);

    const key = b64Decode(alloc, key_b64) catch return returnEmpty(info);
    defer alloc.free(key);
    const msg = b64Decode(alloc, msg_b64) catch return returnEmpty(info);
    defer alloc.free(msg);

    const mac = privacy.hmacSha256With(parseBackend(backend_s), key, msg);
    const enc = b64Encode(alloc, &mac) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privHkdfSha256(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const ikm_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(ikm_b64);
    const salt_b64 = argStr(alloc, info, 2) orelse return returnEmpty(info);
    defer alloc.free(salt_b64);
    const info_b64 = argStr(alloc, info, 3) orelse return returnEmpty(info);
    ...
    const ikm = b64Decode(alloc, ikm_b64) catch return returnEmpty(info);
    defer alloc.free(ikm);
    const salt = b64Decode(alloc, salt_b64) catch return returnEmpty(info);
    defer alloc.free(salt);
    const info_bytes = b64Decode(alloc, info_b64) catch return returnEmpty(info);
    ...
    const enc = b64Encode(alloc, out) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privXchachaEncrypt(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const pt_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(pt_b64);
    const key_b64 = argStr(alloc, info, 2) orelse return returnEmpty(info);
    defer alloc.free(key_b64);
    const nonce_b64 = argStr(alloc, info, 3) orelse return returnEmpty(info);
    defer alloc.free(nonce_b64);
    const aad_b64 = argStr(alloc, info, 4) orelse return returnEmpty(info);
    defer alloc.free(aad_b64);

    const pt = b64Decode(alloc, pt_b64) catch return returnEmpty(info);
    defer alloc.free(pt);
    const key = b64Decode(alloc, key_b64) catch return returnEmpty(info);
    defer alloc.free(key);
    const nonce = b64Decode(alloc, nonce_b64) catch return returnEmpty(info);
    defer alloc.free(nonce);
    const aad = b64Decode(alloc, aad_b64) catch return returnEmpty(info);
    ...
    const enc = b64Encode(alloc, out[0..n]) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privXchachaDecrypt(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const ct_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    ...
    const ct = b64Decode(alloc, ct_b64) catch return returnEmpty(info);
    defer alloc.free(ct);
    const key = b64Decode(alloc, key_b64) catch return returnEmpty(info);
    defer alloc.free(key);
    const nonce = b64Decode(alloc, nonce_b64) catch return returnEmpty(info);
...
fn privRandomBytes(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    privacy.randomBytesWith(host.io, parseBackend(backend_s), buf);

    const enc = b64Encode(alloc, buf) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privHashFile(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const path = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(path);
    var hex: [64]u8 = undefined;
    privacy.hashFile(host.io, path, &hex) catch return returnEmpty(info);
...
fn privHashDirectory(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    var manifest = privacy.hashDirectory(host.io, alloc, path, recursive) catch return returnEmpty(info);
...
fn privVerifyManifest(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const json_in = argStr(alloc, info, 0) orelse return returnEmpty(info);
    ...
    var result = privacy.verifyManifest(host.io, alloc, &manifest) catch return returnEmpty(info);
    defer result.deinit();
...
fn privSecbufAlloc(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const hex = argStr(alloc, info, 0) orelse return returnNumber(info, 0);
...
fn privSecbufProtect(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const alloc = host.gpa;
    const id = argI64(info, 0) orelse return;
    const mode_s = argStr(alloc, info, 1) orelse return;
...
fn privEncryptFile(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const in_path = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(in_path);
    const out_path = argStr(alloc, info, 1) orelse return returnBool(info, false);
    defer alloc.free(out_path);
    const key_b64 = argStr(alloc, info, 2) orelse return returnBool(info, false);
    defer alloc.free(key_b64);

    const key = b64Decode(alloc, key_b64) catch return returnBool(info, false);
    defer alloc.free(key);
    if (key.len != 32) return returnBool(info, false);
    const key_arr: *const [32]u8 = key[0..32];

    privacy.encryptFile(host.io, alloc, in_path, out_path, key_arr) catch return returnBool(info, false);
...
fn privDecryptFile(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    privacy.decryptFile(host.io, alloc, in_path, out_path, key_arr) catch return returnBool(info, false);
    returnBool(info, true);
}
...
fn privSecureDelete(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const path = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(path);
    const passes = argI64(info, 1) orelse 3;
    privacy.secureDelete(host.io, path, @intCast(@max(0, passes))) catch return returnBool(info, false);
    returnBool(info, true);
}
...
fn privStegEmbed(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const carrier = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(carrier);
    const secret_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(secret_b64);
    const secret = b64Decode(alloc, secret_b64) catch return returnEmpty(info);
...
fn privStegExtract(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const encoded = argStr(alloc, info, 0) orelse return returnEmpty(info);
...
fn privTokenize(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const value = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(value);
    const salt = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(salt);
...
fn privGpgEncrypt(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const pt = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(pt);
    const recipient = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(recipient);
    const out = privacy.gpgEncrypt(host.io, alloc, pt, recipient) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privGpgDecrypt(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const ct = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(ct);
    const out = privacy.gpgDecrypt(host.io, alloc, ct) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privGpgSign(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const msg = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(msg);
    const out = privacy.gpgSign(host.io, alloc, msg) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privGpgVerify(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const signed = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(signed);
    const ok = privacy.gpgVerify(host.io, alloc, signed) catch return returnBool(info, false);
    returnBool(info, ok);
}
...
fn privGpgListKeys(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const out = privacy.gpgListKeys(host.io, alloc) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privGpgImport(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const armored = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(armored);
    const out = privacy.gpgImportKey(host.io, alloc, armored) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privGpgExport(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const id = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(id);
    const out = privacy.gpgExportKey(host.io, alloc, id) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privMetaStrip(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const path = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(path);
    privacy.metaStrip(host.io, alloc, path) catch return returnBool(info, false);
    returnBool(info, true);
}
...
fn privMetaRead(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const path = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(path);
    const out = privacy.metaRead(host.io, alloc, path) catch return returnEmpty(info);
    defer alloc.free(out);
    returnString(info, out);
}
...
fn privAnonymousId(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const domain = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(domain);
    const seed_b64 = argStr(alloc, info, 2) orelse return returnEmpty(info);
    defer alloc.free(seed_b64);

    const seed = b64Decode(alloc, seed_b64) catch return returnEmpty(info);
...
fn privPseudonym(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const backend_s = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(backend_s);
    const master_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(master_b64);
    const context = argStr(alloc, info, 2) orelse return returnEmpty(info);
    defer alloc.free(context);

    const master = b64Decode(alloc, master_b64) catch return returnEmpty(info);
...
fn privIsolatedCredential(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const domain = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(domain);

    var cred = privacy.isolatedCredential(alloc, domain) catch return returnEmpty(info);
    defer cred.deinit(alloc);

    const pub_b64 = b64Encode(alloc, &cred.public_key) catch return returnEmpty(info);
    defer alloc.free(pub_b64);
    const sec_b64 = b64Encode(alloc, &cred.secret_key) catch return returnEmpty(info);
    defer alloc.free(sec_b64);
    const id_hex = hexEncodeAlloc(alloc, &cred.key_id) catch return returnEmpty(info);
...
fn privNoiseInitiate(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const pub_b64 = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(pub_b64);
    const remote_pub_bytes = b64Decode(alloc, pub_b64) catch return returnEmpty(info);
    ...
    const result = privacy.noiseInitiateSession(host.io, remote_pub) catch return returnEmpty(info);
    const handshake_b64 = b64Encode(alloc, &result.handshake) catch return returnEmpty(info);
...
fn privNoiseRespond(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const priv_b64 = argStr(alloc, info, 0) orelse return returnNumber(info, 0);
    defer alloc.free(priv_b64);
    const msg_b64 = argStr(alloc, info, 1) orelse return returnNumber(info, 0);
    defer alloc.free(msg_b64);

    const priv_bytes = b64Decode(alloc, priv_b64) catch return returnNumber(info, 0);
    defer alloc.free(priv_bytes);
    const msg_bytes = b64Decode(alloc, msg_b64) catch return returnNumber(info, 0);
...
fn privKeyringCreate(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const path = argStr(alloc, info, 0) orelse return returnNumber(info, 0);
    defer alloc.free(path);
    const password = argStr(alloc, info, 1) orelse return returnNumber(info, 0);
    defer alloc.free(password);

    var ring = keyring.create(host.io, alloc, path, password) catch return returnNumber(info, 0);
...
fn privKeyringOpen(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    var ring = keyring.open(host.io, alloc, path, password) catch return returnNumber(info, 0);
    const id = g_keyring_next_id;
    g_keyring_next_id += 1;
    keyringRegistry().append(alloc, .{ .id = id, .ring = ring }) catch {
        ring.deinit();
...
fn privKeyringGenerate(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const alloc = host.gpa;
    const id = argI64(info, 0) orelse return returnEmpty(info);
    const opts_json = argStr(alloc, info, 1) orelse return returnEmpty(info);
    ...
    const hex = hexEncodeAlloc(alloc, &new_id) catch return returnEmpty(info);
    defer alloc.free(hex);
    returnString(info, hex);
}

fn keyEntryToJson(alloc: std.mem.Allocator, buf: *std.ArrayList(u8), e: *const keyring.KeyEntry) !void {
    try buf.append(alloc, '{');
    try buf.appendSlice(alloc, "\"id\":\"");
    const id_hex = try hexEncodeAlloc(alloc, &e.id);
    defer alloc.free(id_hex);
    ...
    const pub_hex = try hexEncodeAlloc(alloc, &e.public_key);
    ...
    if (e.expires) |x| try listPrint(alloc, buf, ",\"expires\":{d}", .{x});
    if (e.revoked) |r| try listPrint(alloc, buf, ",\"revoked\":{d}", .{r});
    if (e.rotated_to) |rt| {
        try buf.appendSlice(alloc, ",\"rotatedTo\":\"");
        const rt_hex = try hexEncodeAlloc(alloc, &rt);
...
fn privKeyringList(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    for (entry.ring.entries.items, 0..) |*e, i| {
        if (i > 0) buf.append(alloc, ',') catch return returnEmpty(info);
        keyEntryToJson(alloc, &buf, e) catch return returnEmpty(info);
    }
    buf.append(alloc, ']') catch return returnEmpty(info);
    returnString(info, buf.items);
}
...
fn privKeyringGet(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const alloc = host.gpa;
    const id = argI64(info, 0) orelse return returnEmpty(info);
    const key_hex = argStr(alloc, info, 1) orelse return returnEmpty(info);
...
fn privKeyringRevoke(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const key_hex = argStr(alloc, info, 1) orelse return returnBool(info, false);
    defer alloc.free(key_hex);
    const reason = argStr(alloc, info, 2) orelse return returnBool(info, false);
...
fn privKeyringExport(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const enc = b64Encode(alloc, &pub_key) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privPiiDetect(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const text = argStr(alloc, info, 0) orelse return returnEmpty(info);
...
fn privShamirSplit(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const secret_b64 = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(secret_b64);
    const n = argI64(info, 1) orelse return returnEmpty(info);
    ...
    const shares = privacy.shamirSplit(host.io, alloc, secret, @intCast(n), @intCast(k)) catch return returnEmpty(info);
    defer {
    ...
    for (shares, 0..) |sh, i| {
        if (i > 0) buf.append(alloc, ',') catch return returnEmpty(info);
        const enc = b64Encode(alloc, sh.bytes) catch return returnEmpty(info);
        defer alloc.free(enc);
        listPrint(alloc, &buf,
            \\{{"index":{d},"data":"{s}"}}
...
fn privEnvelopeEncrypt(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const data_b64 = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(data_b64);
    const kek_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(kek_b64);

    const data = b64Decode(alloc, data_b64) catch return returnEmpty(info);
    defer alloc.free(data);
    const kek_bytes = b64Decode(alloc, kek_b64) catch return returnEmpty(info);
    defer alloc.free(kek_bytes);
    if (kek_bytes.len != 32) return returnEmpty(info);
    var kek: [32]u8 = undefined;
    @memcpy(&kek, kek_bytes);

    const env = privacy.envelopeEncrypt(host.io, alloc, data, kek) catch return returnEmpty(info);
    defer alloc.free(env.ciphertext);

    const edek = b64Encode(alloc, &env.encrypted_dek) catch return returnEmpty(info);
    defer alloc.free(edek);
    const dn = b64Encode(alloc, &env.dek_nonce) catch return returnEmpty(info);
    defer alloc.free(dn);
    const ct = b64Encode(alloc, env.ciphertext) catch return returnEmpty(info);
    defer alloc.free(ct);
    const dan = b64Encode(alloc, &env.data_nonce) catch return returnEmpty(info);
...
fn privEnvelopeDecrypt(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const env_json = argStr(alloc, info, 0) orelse return returnEmpty(info);
    ...
    const edek_bytes = b64Decode(alloc, switch (edek_v) {
        .string => |s| s,
        else => return returnEmpty(info),
    }) catch return returnEmpty(info);
    defer alloc.free(edek_bytes);
    const dn_bytes = b64Decode(alloc, switch (dn_v) {
        .string => |s| s,
        else => return returnEmpty(info),
    }) catch return returnEmpty(info);
    defer alloc.free(dn_bytes);
    const ct_bytes = b64Decode(alloc, switch (ct_v) {
        .string => |s| s,
        else => return returnEmpty(info),
    }) catch return returnEmpty(info);
    defer alloc.free(ct_bytes);
    const dan_bytes = b64Decode(alloc, switch (dan_v) {
        .string => |s| s,
        else => return returnEmpty(info),
    }) catch return returnEmpty(info);
    ...
    defer alloc.free(pt);
    const enc = b64Encode(alloc, pt) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privStegImageEmbed(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const rgba_b64 = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(rgba_b64);
    const data_b64 = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(data_b64);
    const rgba = b64Decode(alloc, rgba_b64) catch return returnEmpty(info);
    defer alloc.free(rgba);
    const data = b64Decode(alloc, data_b64) catch return returnEmpty(info);
    defer alloc.free(data);
    privacy.stegEmbedImageRGBA(rgba, data) catch return returnEmpty(info);
    const enc = b64Encode(alloc, rgba) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privStegImageExtract(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    defer alloc.free(data);
    const enc = b64Encode(alloc, data) catch return returnEmpty(info);
    defer alloc.free(enc);
    returnString(info, enc);
}
...
fn privAuditCreate(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const key_hex = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(key_hex);
...
fn privAuditAppend(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const event = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(event);
    const data_json = argStr(alloc, info, 1) orelse return returnEmpty(info);
    defer alloc.free(data_json);
    const e = privacy.auditAppend(host.io, event, data_json) catch return returnEmpty(info);
    var buf: std.ArrayList(u8) = .empty;
    defer buf.deinit(alloc);
    listPrint(alloc, &buf, "{{\"index\":{d},\"timestamp\":{d},\"event\":", .{ e.index, e.timestamp }) catch return returnEmpty(info);
...
fn privPolicySetRetention(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const category = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(category);
    const json = argStr(alloc, info, 1) orelse return returnBool(info, false);
    defer alloc.free(json);
...
fn privPolicyRecordConsent(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const user_id = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(user_id);
    const purpose = argStr(alloc, info, 1) orelse return returnBool(info, false);
    defer alloc.free(purpose);
    const granted = argBool(info, 2);
    privacy.policyRecordConsent(host.io, user_id, purpose, granted) catch return returnBool(info, false);
    returnBool(info, true);
}
...
fn privPolicyRevokeConsent(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const user_id = argStr(alloc, info, 0) orelse return returnBool(info, false);
    defer alloc.free(user_id);
    const purpose_opt: ?[]u8 = argStr(alloc, info, 1);
    defer if (purpose_opt) |p| alloc.free(p);
    privacy.policyRevokeConsent(host.io, user_id, if (purpose_opt) |p| p else null) catch return returnBool(info, false);
    returnBool(info, true);
}
...
fn privPolicyErasure(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const user_id = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(user_id);
...
fn privCheckAlgo(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const algo = argStr(alloc, info, 0) orelse return returnEmpty(info);
...
fn privSanitizeFilename(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const name = argStr(alloc, info, 0) orelse return returnEmpty(info);
...
fn privNormalizeTimestamp(info_c: ?*const v8.c.FunctionCallbackInfo) callconv(.c) void {
    ...
    const host = v8_runtime.hostContext(info.getIsolate());
    const alloc = host.gpa;
    const ts = argStr(alloc, info, 0) orelse return returnEmpty(info);
    defer alloc.free(ts);
...
pub fn registerPrivacy(_: *HostContext) void {
    _ = sodium.init();
