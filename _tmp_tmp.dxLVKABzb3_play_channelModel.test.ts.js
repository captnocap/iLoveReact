(() => {
  // cart/editor/play/channelModel.ts
  var PLAY_CHANNEL_TUNING = {
    marketTickMs: 2e3,
    phoneWindowSize: 5,
    maxFeedRows: 32,
    startingCreditsCents: 2367,
    startingMarketPriceCents: 2367,
    marketDeltasCents: [7, -4, 12, -9, 5, 3, -6, 10]
  };
  var GIGS = [
    { id: "fishes", title: "SWIMMING WITH THE FISHES", strap: "wet work", detail: "Recover a sealed package before the tide turns witness.", client: "South Jetty Mutual", payoutCents: 8400, risk: 3, tone: "cyan" },
    { id: "felony-finder", title: "FELONY FINDER", strap: "skip tracing", detail: "Locate a parole ghost using only public mistakes and old favors.", client: "Vera Bail Group", payoutCents: 5600, risk: 2, tone: "amber" },
    { id: "flockbook", title: "FLOCKBOOK", strap: "like. obey. repeat.", detail: "Seed a grievance, steer the replies, and sell the resulting panic.", client: "Patriot Growth LLC", payoutCents: 3200, risk: 2, tone: "cyan" },
    { id: "drown", title: "DROWN", strap: "liquidation services", detail: "Sink a reputation without leaving fingerprints on the timeline.", client: "DROWN Official", payoutCents: 6100, risk: 3, tone: "green" },
    { id: "pills-r-us", title: "PILLS-R-US", strap: "your pain. our profit.", detail: "Move a mislabeled sample case across three compliance zones.", client: "ReliefDirect", payoutCents: 7700, risk: 4, tone: "violet" },
    { id: "spray-pray", title: "SPRAY AND PRAY", strap: "urban renewal", detail: "Replace a rival mural before the morning commuter cycle.", client: "Civic Color Front", payoutCents: 2900, risk: 1, tone: "violet" },
    { id: "shoplifter", title: "SHOPLIFTER", strap: "lift more. pay less.", detail: "Recover inventory the store insists it never stocked.", client: "Aisle Nine Union", payoutCents: 4500, risk: 2, tone: "amber" },
    { id: "warrant-buffet", title: "WARRANT BUFFET", strap: "all you can flee", detail: "Erase one camera chain while every precinct is looking elsewhere.", client: "Public Defender DAO", payoutCents: 9200, risk: 5, tone: "red" },
    { id: "why-c", title: "WHY-C", strap: "vitamin chaos", detail: "Redirect a supplement shipment and preserve the wellness narrative.", client: "Why-C Holdings", payoutCents: 4800, risk: 2, tone: "cyan" },
    { id: "prophet-margin", title: "PROPHET MARGIN", strap: "predict. prey. profit.", detail: "Make tomorrow's rumor true before today's market closes.", client: "Oracle Street", payoutCents: 8600, risk: 4, tone: "green" },
    { id: "alllots", title: "AILLOTS", strap: "bid low. win souls.", detail: "Depress one parcel auction without disturbing the neighboring lots.", client: "AllLots Municipal", payoutCents: 5100, risk: 3, tone: "amber" },
    { id: "condemnation", title: "CONDEMNATION", strap: "clearance by neglect", detail: "Turn a livable block into an emergency redevelopment opportunity.", client: "Renewal Partners", payoutCents: 12400, risk: 5, tone: "red" },
    { id: "squatter", title: "SQUATTER", strap: "live rent free", detail: "Hold a property long enough for ownership to become negotiable.", client: "Tenants Unknown", payoutCents: 6800, risk: 3, tone: "green" },
    { id: "organ-trail", title: "ORGAN TRAIL", strap: "donate or die", detail: "Escort a temperature-sensitive cooler through private checkpoints.", client: "Mercy Logistics", payoutCents: 11100, risk: 5, tone: "amber" },
    { id: "drought", title: "BEYOND A REASONABLE DROUGHT", strap: "stay thirsty", detail: "Make a water audit arrive at the most profitable conclusion.", client: "Regional Aqua Board", payoutCents: 7300, risk: 3, tone: "cyan" },
    { id: "terms", title: "TERMS OF WORSHIP", strap: "click to agree", detail: "Recover a congregation list before the next terms update.", client: "Blessed Platform", payoutCents: 4200, risk: 2, tone: "violet" },
    { id: "kyc", title: "KYC", strap: "know your customer", detail: "Prove a customer exists without introducing them to reality.", client: "Identity Orchard", payoutCents: 5900, risk: 3, tone: "cyan" },
    { id: "public-offender", title: "PUBLIC OFFENDER", strap: "taxes are theft", detail: "Deliver one sealed civic record to the courthouse basement.", client: "Concerned Taxpayers", payoutCents: 6600, risk: 4, tone: "amber" },
    { id: "hot-fixed", title: "HOT FIXED", strap: "we fix it hot", detail: "Patch a deployed kiosk before its audit log catches up.", client: "Emergency Systems", payoutCents: 7900, risk: 4, tone: "red" },
    { id: "genuinely", title: "GENUINELY", strap: "be real. for a fee.", detail: "Manufacture authenticity for a creator with declining engagement.", client: "Human Presence Co.", payoutCents: 3700, risk: 1, tone: "green" }
  ];
  var INITIAL_FEED = [
  ...
  var INITIAL_NOTICES = [
    ...
    { id: "notice-warrant", title: "WARRANT ALERT", detail: "3 active warrants \xB7 click to review", tone: "red" },
    { id: "notice-payment", title: "PAYMENT PENDING", detail: "AllLots escrow awaiting proof", tone: "amber" },
    { id: "notice-reputation", title: "REPUTATION DROP", detail: "Complaint verified \xB7 -15 social credit", tone: "red" }
  ];
  function bump(revisions, channels) {
  ...
  function formatCredits(cents) {
    ...
    return `${sign}\xA5 ${Math.floor(abs / 100).toLocaleString()}.${String(abs % 100).padStart(2, "0")}`;
  }
  function channelTextureKey(channel, owner = "public") {
    return `criminal-careers:${channel}:${owner}`;
  }
  function initialPlayChannelState() {
    return {
  ...
  function visiblePhonePosts(state) {
  ...
  function dirtyChannels(before, after) {
    const channels = [];
  ...
  function playChannelReducer(state, action) {
    switch (action.type) {
      case "select-gig": {
      ...
      case "accept-selected": {
        ...
        const notice = {
          ...
          detail: `${gig.title} \xB7 ${formatCredits(gig.payoutCents)} on proof`,
          tone: gig.tone
        };
        return {
          ...state,
      ...
      case "complete-active": {
        ...
        return {
          ...
          feed: [post, ...state.feed].slice(0, PLAY_CHANNEL_TUNING.maxFeedRows),
          notices: [{ id: `notice-paid-${sequence}`, title: "PAYMENT RECEIVED", detail: `${formatCredits(gig.payoutCents)} \xB7 ${gig.client}`, tone: "green" }, ...state.notices].slice(0, 6),
          revisions: bump(state.revisions, ["gigwork", "phone", "identity"])
        };
      }
      case "tick-market": {
  ...
  // cart/editor/play/channelModel.test.ts
  var passed = 0;
  var failed = 0;
...
`));
  ...
  test("texture identity is stable across revisions and formatting is integer-safe", () => {
    ...
    assert(formatCredits(2367) === "\xA5 23.67", `unexpected positive format: ${formatCredits(2367)}`);
    assert(formatCredits(-405) === "-\xA5 4.05", `unexpected negative format: ${formatCredits(-405)}`);
  });
  log(`
