...
`;
  ...
  // cart/editor/worldBible/controller.test.ts
  var passed = 0;
  var failed = 0;
...
`));
  ...
  var MapPort = class {
    ...
    list(path) {
      ...
      return [...this.files.keys()].filter((entry) => entry.startsWith(prefix) && !entry.slice(prefix.length).includes("/")).map((entry) => entry.slice(prefix.length));
    }
    ...
    writeAtomicIfUnchanged(path, expected, source) {
      if ((this.files.get(path) ?? null) !== expected) return "changed";
      if (this.failWrites) return "failed";
      this.writes.push(path);
      this.files.set(path, source);
      return "written";
    }
  };
  function sourceFor(ref, name, kind = "business") {
    return `# ${name}
...
`;
  ...
  function draftFor(ref, name, kind = "business") {
    return {
      kind,
      ref,
      name,
  ...
  test("legacy recovery drafts migrate missing authorText to empty", () => {
    const port = new MapPort();
    const legacy = draftFor("mechanic.legacy", "Legacy", "mechanic");
  ...
  test("startup restores only an interrupted native claim pair", () => {
    ...
    assert(port.files.has(temp) && port.files.has(`${temp}.previous`), "forensic recovery artifacts were deleted");
  });
