// rle.ts — the shared row-RLE grid codec for workspace-style carts.
//
// Promoted out of cart/cutout/ (where it began as "one row-RLE codec for
// the whole cutout cart") into the workspace layer, since it is general
// grid compression with no cutout specifics. Every persisted grid in a
// workspace cart — cutout's SQI matrices and brush masks, hmsc-int's
// per-chunk tile / height / zone buffers, pixel-icon JSON — round-trips
// through this module. cart/cutout/rle.ts now re-exports from here.
//
// Wire shape:
 ...
 *  mask which is sized to the SOURCE image, and by hmsc-int's per-chunk
 *  tile / height / zone buffers. */
