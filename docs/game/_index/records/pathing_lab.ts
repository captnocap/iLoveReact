...
export const pathing_lab: DocIndex = {
  ...
  interfaces: [
    {
      ...
      kind: 'host_fn',
      sourceFile: 'framework/v8_bindings_pathing.zig',
    ...
    },
    {
      ...
      kind: 'host_fn',
      sourceFile: 'framework/v8_bindings_pathing.zig',
      description: 'A* + waypoint simplification; returns into reused scratch.',
      consumers: ['runtime/pathing.ts'],
      status: 'live',
    },
    {
    ...
    },
    {
      ...
      sourceFile: 'cart/pathing_lab/index.tsx',
      description:
        'Post-pass snapping every road waypoint to the trio marked center line, deriving the side from DIRECTION OF TRAVEL (right-hand rule), not from whichever column A* wandered through. A position-based snap pinned flow-neutral crosswalk points to the wrong lane line (the |Vxx^| swerves).',
      status: 'lab',
    },
    {
      ...
      sourceFile: 'cart/pathing_lab/index.tsx',
      description:
        'Post-pass replacing all in-box waypoints with the single lane-line apex (exit lane column x entry lane row), computed from the BOX geometry + entry/exit directions, never from raw waypoint coordinates. Probe-verified: coordinate-derived 8/41 routes violated; box-derived apexes 0/89.',
      status: 'lab',
    },
    {
      name: 'buildCarRoute',
  ...
  hazards: [
    {
      name: 'Lane discipline lives cart-side, not in the host',
      purpose: ['pathing', 'vehicle', 'maintenance'],
      description:
        'snapToLaneCenters + straightenJunctions are cart-side JS over host paths and encode the road grammar hardest-won lessons (two probe-verified bug histories in comments). If hmsc adopts host pathing for traffic, this logic should move into the host or runtime/pathing.ts rather than be copy-pasted per cart.',
      evidence: [
        'pathing_lab.md quirks; comments at snapToLaneCenters and straightenJunctions',
        'probe-verified 8/41 vs 0/89 in straightenJunctions',
      ],
      fix: 'Promote snap + apex passes into the host or runtime/pathing.ts before any per-cart copy.',
      severity: 'high',
    },
    {
