...
export const editor_painted_placements: DocIndex = {
  ...
  summary:
    'Which mesh, UV document, immutable atlas-start reset, editable atlas total, infinite image workspace, derived UV guide, coverage-cleaned raster, compiled shared skin atlas, and collision shape a saved/placed authored model uses (req_2832/2833/2930/3133/3328/3329/3362/3431/3515/3520/3522/3523/3524/3525/3526/3527/3528/3529/3530/3537/3544/3545/3546/3547): compatible self-contained GLBs automatically adopt their embedded base-colour image + exact source UVs and register one fingerprint-deduplicated Imported Texture full-look variant. UV corners move freely in signed workspace coordinates, Ctrl-drag silhouette marquee selection collects many UV islands in one native highlight pass, UV mode keeps source images visible beneath the editable graph, finite model-atlas samples outside the image alpha-discard, and content-addressed native-pixel image layers explicitly compile to their smallest transparent finite union while remaining separately editable and lockable. A visible UV-total W/H draft independently scales absolute X/Y texels while preserving normalized UV placement, so differently sized generated wireframe results can retain native pixels. base.paint.json v4 cold-restores every exact face-corner UV; uv-reset.json preserves the atlas generation\'s original signed corner table through later saves and restores it as one undoable, always-reachable context action; welded corner identity stitches selected broken islands to one fixed active island, authored quad-aware edges export as an atlas-sized transparent uv-wireframe.png, finalized source PNGs neutralize texels outside exact UV coverage, and Compile Shared Atlas best-fit packs independent paint looks. Placement uses the full-res meshdoc with the painting\'s UVs rebound onto it except when painting a quality-DECIMATED display. Visible full-res saved Outliner ranges bake both bounded coarse boxes and exact immutable player-collision triangles; painted.json stamps which doc revision owns a decimated exported look; every save also persists mesh/collision.blob (RJCB v1).',
  interfaces: [
    {
    ...
    },
    {
      ...
      sourceFile: 'cart/editor/data/modelPackageStore.ts',
      description:
        'req_3544/3545: the first exact atlas save writes every face corner to atlases/uv-reset.json in signed workspace coordinates; normal saves preserve that record byte-for-byte, while Create/Remake Paint Atlas explicitly captures a new generation. RMB → Texture Atlas → Reset UV Layout applies the complete table through __model_uv_geometry_apply as one append-only reset UV layout journal action, immediately persists the restored current state, and refuses topology/count mismatch. Workspace coordinates keep the reset stationary across finite image-layer crop origins. Legacy packages lazily seed from their last matching base.paint v4 cornerUv, avoiding model reimport; pre-v4 packages can only adopt their current exact live layout. The action menu pins fixed-height labels/details to one line, budgets all eight Texture Atlas rows on first paint, then uses its measured onLayout height for edge clamping so wrapped text or future rows cannot hide Reset beyond the viewport.',
