  ...
  interfaces: [
    {
      ...
      description:
        'req_3548/3549: planRepeatedUvStacks is a mutation-free whole-layout evaluation. Coarse buckets use triangle/point counts, authored-face and welded-corner partitions, plus exact scale or normalized aspect; full signatures then require the complete authored UV triangle graph to agree under quarter-turn/reflection transforms, so equal bounding boxes alone never authorize a stack. Exact mode keeps texel scale within 0.01px. Normalize removes uniform scale only for members at or below its editable summed authored-triangle UV area (default 4,096px²), reports larger congruent surfaces as protected, and adopts the largest eligible member\'s exact corners. If fewer than two members remain eligible, no family is emitted. UvEditor yields once for a visible scan state, freezes both alternatives, reports islands→unique footprints/families/moves/density changes/protected surfaces, and gates one stack UV islands journal commit behind Apply, + Wire, or + AI Guide. Export actions send the reviewed local corners into ModelView\'s door so state scheduling cannot export the stale pre-stack graph. Face/group/welded identities remain source-owned. Uniform pack and guide numbering share groupUvTextureFootprints, preserving confirmed stacks as one unit/number. A saved Bed_003 diagnostic reduced 110 islands to 60 footprints across 32 families; a 1,024-island regression covers deterministic bulk grouping.',
