...
export const editor_painted_placements: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'req_3530/3537: mesh_import surfaces one encoded embedded image only when every emitted primitive can honestly share it: TEXCOORD_0, one baseColorTexture image/common factor, finite unit UVs, and no vertex-colour/alternate-texcoord/KHR_texture_transform semantics. Geometry remains loadable when that contract fails. __mesh_load_file metadata-guards and decodes the image, bakes the common factor, and snapshots absolute source corners before setPaintTarget rewrites the parser UV lanes. After face grouping/layout it imports the image through the opaque finite-atlas boundary, applies that owned source snapshot, and stashes the final active edit copy. ModelView skips its fallback colour flood and ensureImportedTexturePaintVariant stores an Imported Texture full look keyed by model SHA + glTF image index. UV provenance v2 refreshes the known-bad pre-fix v1 automatic row in place on the next source load; Save-back strips provenance first, so neither that migration nor future pristine-source capture can rewrite an authored variant. New imports also write the source look as base, while packages with established paint keep their displayed base.',
