...
export const editor_painted_placements: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'req_3515/3516/3517: stitchUvIslands consumes the cornerVertices ids already published for UV corner colors. It cancels internal edges to recover welded boundaries, keeps the white active island fixed, then walks every selected island reachable by a shared topology edge (or one unambiguous boundary vertex). A handed similarity fit preserves the moving island while exact seam endpoints make the host rebuild one connected island; unrelated selections stay put and out-of-atlas fits fail closed. UvEditor keeps STITCH visible in the toolbar and exposes Stitch Matching Seams as a direct RMB row carrying the detected island count; it records the entire sweep as one append-only stitch UV seams history action. WIRE PNG rasterizes the same authored face edges plus heavier island boundaries onto alpha-zero RGBA, so authored quads do not regain resident triangle diagonals; writeModelUvWireframe atomically writes atlases/uv-wireframe.png and the UI copies the proven absolute path.',
