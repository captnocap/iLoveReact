...
export const editor_painted_placements: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'req_3530: mesh_import surfaces one encoded embedded image only when every emitted primitive can honestly share it: TEXCOORD_0, one baseColorTexture image/common factor, finite unit UVs, and no vertex-colour/alternate-texcoord/KHR_texture_transform semantics. Geometry remains loadable when that contract fails. __mesh_load_file metadata-guards and decodes the image, bakes the common factor, imports it through the opaque finite-atlas boundary, restores source corner UVs, and stashes the final active edit copy. ModelView skips its fallback colour flood and ensureImportedTexturePaintVariant stores an Imported Texture full look keyed by model SHA + glTF image index. New imports also write it as base; packages with authored paint keep their existing displayed base. Save-back strips provenance so an edited row cannot suppress future pristine-source capture.',
