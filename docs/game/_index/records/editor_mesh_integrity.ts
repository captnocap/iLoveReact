  ...
  interfaces: [
    {
      ...
      description:
        'ARMED at journalCommit (every accepted topology op) and after a successful journalStep (undo/redo); RUNS at the next meshActionDrain — deferred because some ops (loop cut renormalize) settle range tables after committing, so a commit-time check reads half-settled state as a fault. Gathers displayed + hidden-part face groups, runs mesh_journal_log.analyze (rangesValid / unownedFaces / multiplyOwnedFaces), heals only what is provable — ensureDisjointPartRanges for overlaps, compactOccupiedPartRanges for declarations owning no face — and on any fault logs [mesh-integrity] and enqueues an integrity_alert event with declared part counts around the heal. Never guesses ownership; never blocks the op; meshJournalClear disarms across document loads.',
