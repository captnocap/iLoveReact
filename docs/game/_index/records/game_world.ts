...
export const game_world: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'The V20 world concern in ONE registration: grid edits as events (cellPlaced/cellRemoved/regionFilled/triggerSet/landformPlaced/landformRemoved/respawnArmed) folding into the WorldGridState snapshot the game loads; unknown future kinds pass through untouched (schema evolution by addition). The V24 placed-piece family joined BY ADDITION (the Creative Build route): piecePlaced/pieceRemoved/pieceEditSet/prefabDefined/prefabStamped fold into `pieces` (THE placed-piece truth; ids minted by the materializer as bp_<seq> so replay reproduces them) and `prefabs` (world-saved clone-from-world definitions). A prefab stamp is ONE event landing as its semantic pieces (the see-through law); the materializer is tolerant, the authoring boundary (validatePlacement) is strict. PLACEDEDIT-0613 (req_0748) added in-place pose/type edits keyed by id so a detached panel (the iso pane PAINT tab) can edit a clicked piece without re-minting its id and thrashing the selection: pieceMoved (x/y/z/yaw), pieceSwapped (catalog type — keeps skin, drops an editless cutout like the buildings store), and pieceEditSet now accepts edit:null to CLEAR the cutout back to none.',
      status: 'live',
    },
  ],
  patterns: [
