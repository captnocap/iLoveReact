...
export const hmsc_int: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'TEXPORT-0606 (2026-06-06, USER ASK "properly port that into the correct space"): the texture pipeline MOVED from cart/hmsc/render3d behind the captured ground floor\'s own door — a faithful move, not a rewrite (export names, custom: ids, the custom-textures store key, and behavior unchanged; old saves resolve). shaders.ts (was textureShaders.ts) = the canonical tunable-WGSL recipe catalog; materials.ts (was customTextures.ts) = the stored materials Materialize freezes into the shared hmsc store; registry.tsx (was textures.tsx) = THE one texture registry (TextureDef/TEXTURE_REGISTRY/allTextures/textureById/TextureCapture) every face/tile/part samples by id. Follows the game/world pattern (own module, not a 20th ruled game/index.ts door); consumers use @game/textures subpaths (TextureStudio, ShaderLab, TexturePreview, ObjectsTab, editors/cutout) and the legacy renderer cart/hmsc/render3d/parts.tsx imports the registry FROM hmsc-int (the V15 compile direction). GAP edges marked at the import sites: roadTileFill/fillShader WGSL stays with the W-2 render fills; buildingSkins (REACT_TEXTURES) retires WITH the hand-coded buildings; design.PerceptionState + state/gameState store wires are V15. DECALEDIT-0606 added the DECAL source behind the same door: decal.ts (the DecalDoc model — boundary validator that degrades corrupt records to null, billboard-led size presets, text nodes carrying fontFamily/fontWeight/letterSpacing so future graffiti faces are a host family addition with zero schema work) + decalRender.tsx (DecalSurface — the doc scaled to any capture size; x/y stretch independently, glyphs scale by the min axis); materials.ts stores decal records beside shader records (same custom: ids + store key; saveDecalTexture upserts by id for the re-edit law) and registry.tsx hydrates them as react-source TextureDefs, so TextureCapture/pickers need no decal knowledge. The V24 piece-face and voxel-item texture slots land on this registry. See game/textures/CAPTURE.md. DECALPIX-0610 (2026-06-10, USER ASK req_0566 "inside of the compiled game output i am not getting these to show up"): decals reach the COMPILED game by V29 bake-by-execution — a decal has no WGSL recipe, so the headless bake dropped decal faces to flat color. DecalPixelBaker.tsx (EditorShell root) renders any stale saved decal into an offscreen StaticSurface, reads it back via the __capture_surface_pixels host door (v8_bindings_capture.zig → gpu.readbackStaticSurface, always-RGBA self-describing payload), writes the pixels to disk, and persists a tiny {w,h,docHash,file} payload on the record (saveDecalPixels; re-save keeps stale pixels until the re-bake lands so a mid-edit compile ships the old look, never a blank). compile/worldGeometry.ts internMaterial loads the file and encodeMaterials ships pixel-PackBits RLE in the MATERIALS lump\'s optional PIXS tail (older payloads parse unchanged); constructor.zig decodeMaterialPixelTail decodes and material_tex.materializePixels uploads under the same wmat-<i> key shader materials use — textured batch + whitened rows + streaming all shared. Unbaked decals warn by id at bake time and ship flat until the editor runs once. DECALPIXFILE-0610 (req_0569 spam + req_0572, same day): inline payloads NEVER persisted — the shared localstore caps values at 8KB (storage/localstore.zig MAX_VALUE; hostLocalstoreSet swallows failures) — so the baker re-baked forever and the compile found nothing. Pixels live as FILES under cart/hmsc-int/data/decal-pixels/<id>.json (gitignored derived cache) in the repo\'s rows-of-runs pixel JSON (USER: "if you need a format look here → cart/pixel_icons/" — {width,height,palette,rows}, [count,value] runs, null transparent, #rrggbbaa alpha), read by both hosts via plain __fs_read; the baker VERIFIES the store round-trip after each save and parks with one console.error instead of spinning. P4: decalPixels.test.ts 9 cases (both codecs, fs store/load, refused-write park, staleness) + worldGeometry.test.ts PIXS-tail layout + no-pixels byte-compat.',
