...
export const hmsc_int: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'The V20 persistence layer — openStore(rootDir) is the only door. Per-concern append-only streams (data/streams/<name>.jsonl), ONE total cross-session undo chain (global seq across all streams; an undo point is a log position — stateAt(seq) reads as-of, history never rewrites), materialized snapshots stamped with their chain position (the game/compile loads snapshots, never history). The incompleteness guard is the API: defineStream demands name AND initial+apply in one registration. apply(state, event, seq?) — the store passes each event\'s log position; two-arg materializers stay valid. Content gitignored; backup = exportBackup() (streams + manifest). Host gap: no __fs_append binding yet → read+concat+write. TOLERANCE (STOREDB-0606 step 0, the sessions.jsonl:884 outage): the reader NEVER throws — a corrupt/partial record anywhere is skipped + logged loudly + quarantined in memory (store.quarantine(), byte-faithful {path,line,raw,trailing}); the fold continues with every valid record; the file is NEVER rewritten (no repair writes). The writer\'s seam guard starts an append on a fresh line when the file ends in torn bytes (no final newline) — the un-guarded splice is what turned a tolerated tear into the :884 mid-file corruption. P4 suite data/data.test.ts rides rjit game verify.',
      status: 'live',
    },
    {
