...
export const hmsc_int: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'Route /test (~266): walk the staged world — the FIRST real consumer of the @game ground floor (rewired 2026-06-04, contract: cart/hmsc-int/TestRoute.REWIRE.md; SUBSTRATE-0605 2026-06-05: the embodied drop-in extracted to Embodied.tsx, this lineage was the authority). The route keeps only its mode layer over useEmbodiedPlayer: the backtick GAME_COMMANDS console session (live speed owner — gv_speed drives the real walk/run; pv_teleport adopts back through adoptPose; open console gates key reads via options.isTyping), the RMB ADS aim opt-in (options.aim), and the [probe-player-model] gait/rig continuity diagnostic. Remaining cart/hmsc reads marked GAP(W-1 world grid / W-2 world render / W-3 game sky) live in the substrate now.',
