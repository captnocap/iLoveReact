...
export const hmsc_int: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'WORLDWIN-0611 (review §6/§10.2, USER: "Do the real framework work"): the compiled world in a SECOND OS window on the full wgpu pipeline. Framework: gpu.createWindowSurface/configureExtraSurface (extra surfaces, same device), scene3d.DetachedTarget/renderDetached (caller-owned RT outside the per-frame pool; drawScene is encoder-self-contained), framework/gpu/world_window.zig (SDL window + blit pass + event routing; engine loop calls routeEvent + frame). Door rides the compiled-world ingredient: __compiled_world_window/_close/_status. Cart: popOutCompiledWorld() (POP OUT on the /compiled header); Compile calls reloadCompiledWindowIfOpen() — paint → Compile → the window takes the fresh gamefile live, zero route flips. In-window: click captures mouse, WASD walks (process-wide key state), Esc releases, RMB aims; events consumed so editor hotkeys never fire while walking. Runtime mounts under reserved node id 0xFFFFFF01. The in-process <Window> primitive remains 2D-only by design — this is the wgpu path. PROPUSE-COMPILED (req_0624, 2026-06-11): the compiled game carries /test\'s prop interaction capability — the INTERACTABLES map lump (id 16, compile/worldInteractables.ts: archetype table per prop kind + thin instance refs, the factored shape; collected in pushPropGeometry so build-piece AND painted props ship) decoded by constructor.zig into Scene.interactables; world_loader.stepInteract mirrors PlayRoute\'s interact frame (reach 2.2m, E edge, search timer w/ move-cancel + searched set + locked denial, seat pins player + posture until WASD/Space); sit/lay are baked player clips 3/4 (playerModel.ts, skeleton posture actions); the HUD (prompt pill / loading bar / notice) draws natively through the shared 2D batches — embedded+standalone right after the world quad (image segment boundaries put it on top), the pop-out via world_window.frame post-main-frame batch use + gpu.setGlobalsScreenSize/restoreGlobalsScreenSize; standalone main() inits the TextEngine. Proof: rjit game shot GREEN, 14 archetypes / 30 interactable props parsed at boot; P4 compile/worldInteractables.test.ts 3/3. KICKPROP-COMPILED (req_0625, 2026-06-11): the compiled game gets /test\'s dynamics — DYNAMIC_PROPS lump 17 (compile/worldDynamicProps.ts: sphere-body recipe + LOCAL 13-float render parts; pushPropGeometry routes dynamics kinds OUT of the static instance buffer); world_loader reserves an entity section in the physics input (PhysicsColliders.entity_capacity, [header][entities][rects][oriented], every rect writer/reader shifted) and steps bodies through the SAME game_physics.step (player kick, bounce, sphere-sphere); bodies render as live per-frame nodes (player-model pattern); seated = intent-less ghost step so balls keep rolling. HEIGHTFIELDS (USER report): physics.zig\'s entity loop now samples heightfieldGroundAt (bodies land on painted terrain — fixes /test too), and the bake applies liftPropsToTerrain (37033d24f\'s per-prop lift) after the building lift in packageMap so stored y=0 props ship resting ON painted hills; dynamic bodies also spawn at max(anchor, terrain)+radius. Proof: shot GREEN, "dynamics layer: 5 kickable props" at boot; P4 worldDynamicProps.test.ts 3/3, test-game-physics green.',
      status: 'live',
    },
    {
    ...
    },
    {
      ...
      description:
        'PANELGRAMMAR-0610 (review §11.4): pure PanelSpec analysis consulted by the one renderer (fields.tsx PanelGroups) — warns loudly once per offending panel shape, render continues. G1 repeated group shapes illegal (groupSignature detects; factor into one group + selector), G2 one color system per panel (caps: color fields/panel, quick-picks/field without wheel|range), G3 verb caps (chip walls demand t:pick, req_0184), G4 undo/redo/save render once. Named caps in PANEL_GRAMMAR_CAPS. PanelGroup grew tier:debug (collapsed by default). First fix under the law: buildings per-kind GLOBAL groups folded to ONE SKINS·GLOBAL group + class enum; its color field gained wheel+range. P4: shell/panelGrammar.test.ts (6).',
      status: 'live',
    },
    {
      name: 'WorkbenchSlider on the host <Slider> (SLIDER-0611)',
      purpose: ['ui', 'input'],
      kind: 'component',
      sourceFile: 'cart/hmsc-int/shell/fields.tsx',
      description:
        'L1 closed: WorkbenchSlider renders through the framework’s host-driven <Slider> primitive (runtime/primitives.tsx; engine type "Slider"), so every num field, the shader lab, and the cutout tool rail upgraded in ONE place. The engine owns the thumb while the button is down (V23/movePlacement law applied to scrubbing): engine.zig slider drag updates the pool node’s slider_value and repaints with zero JS in the loop; paintSlider draws track/fill/knob host-side (track = style background, fill = color). JS hears __dispatchSliderChange (throttled ~60Hz, label mirror) and __dispatchSliderCommit (once on release — the authoritative settle; WBCHAR-0606 commit-on-release kept). Mid-drag sliderValue echoes are ignored host-side (slider_dragging gate in v8_app.zig applyProps). Nonlinear toTrack/fromTrack consumers keep working — host runs the 0..1 track domain, mapping stays JS-side.',
      consumes: ['__dispatchSliderChange', '__dispatchSliderCommit'],
      dependsOn: ['runtime/primitives.tsx <Slider>', 'framework/engine.zig paintSlider + slider drag'],
      status: 'live',
    },
    {
    ...
    },
    {
      ...
      description:
        'WIRING ONLY since SHELLFOLD-0611 (review §2: the ~1,000-line EditorShell cut along its four seams). The shell keeps route composition, chrome + popovers, the event-log trace, the world/buildings session opens, the tunables boot fold, and compile; the concerns live in editors/world/: useMapSession.ts (MapPayload v2 + buildPayload/applyPayload + useWorkspace autosave + undo snapshot hooks + open/new/rename/delete + the VIEWRUNAWAY/MAPGONE view-sanity laws + every per-map twig + placement STATE + tile overrides + camera seeds; boot reads ONE envelope — the view seed and legacy-piece probe shared a single parse, review §1 partially fixed), useBuildUndo.ts (build-stream commits scoped to their owning map, commitMany batches as ONE undo step, the Ctrl+Z piece reconciler pieceValueKey/reconcileBuildPieces — undo APPENDS, V20; the four commitMany structural casts deleted, it IS the RouteSession contract), usePlacements.ts (the place layer CRUD verbs + the canvas place API), previewWorld.ts (assemblePreviewWorld — a PURE (floors, placements, kind textures) → GameState compiler shared by the iso pane, /test, and Compile). MAPGONE2-0605 view2d restore law unchanged, now inside useMapSession. Regression: editors/world/mapload.test.ts + twigrestore + viewrunaway.',
      dependsOn: ['editors/world/useMapSession.ts', 'editors/world/useBuildUndo.ts', 'editors/world/usePlacements.ts', 'editors/world/previewWorld.ts', 'useWorkspace', 'QuadSplit', 'shell/chrome (Chrome)', 'PaintCanvas', 'IsoAuthor', 'PropertiesPanel', 'RightPanel'],
      status: 'live',
    },
    {
      name: 'AGENTS.md',
    ...
    },
    {
      ...
      description:
        'BUILDMODE-0605 (2026-06-05): build the map WHILE PLAYING — V24 Fortnite-Creative semantics on the embodied drop-in. SUBSTRATE-0605: the player (V23 node-bound native camera, GAME_PHYSICS host step, GAME_WORLD colliders + heightfields, GAME_INPUT key transport, V2 figure) is NOT route code — BuildRoute consumes the ONE shared substrate (cart/hmsc-int/Embodied.tsx, useEmbodiedPlayer + EmbodiedScene/Captures/MouseSurface) and feeds it EmbodiedWorldExtras (placed-piece solids + ramp/stairs heightfield slopes after the terrain bake), onFrame (snap re-resolve) and onTap (place). This also FIXED the launch camera (CAMGONE-0605: the original copy bound via module-level bindFirst with no nativeCamera node — never engaged). MOUSE CAPTURE (addendum 4): the substrate consumes the mouse — raw-motion look, captured click = ALWAYS place, Esc frees the mouse for the palette UI; the drag/click pixel-slop heuristic is dead. The builder layer: crosshair (the solved camera\'s screen-center axis via the substrate-exported PLAYER_CAMERA — the crosshair law) → snap target → registry-driven palette (GAME_BUILD kinds/catalog/prefabs; RULED hotkey order leads the display: 1 floor · 2 wall · 3 ramp · 4 roof (USER VERDICT, addendum 2), then registry order, 0 prefabs, [ ] variant) → ghost preview (piece or whole prefab decomposition at P2 ghost opacity) → click places → R rotates → E cycles the WallEdit vocabulary on the targeted piece → X removes → P marks → named prefab (prefabFromPieces) → palette stamps (decompose per the see-through law). EVERY interaction = ONE labeled session commit on the WORLD channel (editors/sessions); the world stream\'s materialized state is the ONE placed-piece truth (re-read after each commit, no second copy); live P2 knobs (reach/ghost/march) in the in-route tuning panel. snap.ts = pure crosshair→snap resolution (nearest of piece-face vs ground in reach; grid cell-centers, edge pins to the nearer grid line and runs along it, surface mounts proud of the face, free raw; top faces stack storeys, side faces place beside). ONE MODEL, TWO VIEWS: placements are plain worldStream events — \'/build\' exists only as the session\'s route label. P4: snap.test.ts 11 + commits.test.ts 3 (one-commit-per-placement, stamp-is-one-commit, undo-point steps back) + viewport.test.ts 5 (the consumption-layer camera/capture/hotkey proof: both routes pinned to the substrate, the CAMGONE bindFirst shape banned, capture-mode look pinned, ruled hotkeys pinned), editors suite root, verify GREEN. Surfaced (CAPTURE.md): global-not-per-map pieces (V20 scoping question), window/brokenWindow keep collision until a mantle system, no-lintel portals, stepped-box ramp visuals over true slope collision, overlap allowed, edge snap owns its orientation. PLAYFOLD-0605 (2026-06-05, USER ASK "its the same game … fold it so that i can just toggle between them with the F keys like f1 f2"; same day the /build URL retired as a dupe of the folded surface): BuildRoute.tsx + TestRoute.tsx folded into editors/play/PlayRoute.tsx — ONE route (/test, the chrome Play button), mode is PlayRoute\'s own state, F1/F2 flip it in place with NO remount: pose, camera, mouse capture, the backtick console, and the placed pieces carry across the toggle. \'/build\' survives only as the session channel label + twig storage keys (names, not URLs). The build layer above is UNCHANGED inside the fold, rendered/gated by mode; build hotkeys additionally gate while the console is open (the console now opens in both modes); placed pieces render AND collide in BOTH modes (testing what you built is the toggle\'s point). snap.ts + the P4 suites stay in editors/build/. REQ-0647 (2026-06-12): door/doorway/garage/window/brokenWindow methods are CATALOG WALL ROWS carrying a defaultEdit (placed.placementFor stamps the cut on every placement site — F2, iso click, iso drag-paint; edge snap = alignment free; NEVER prefabs, USER ruled), and the ELEVATOR is a first-class vertical-link PIECE KIND: one 3m storey module per placed piece, stacked storeys = a shaft with a stop per storey (game/build/elevators.ts), open-front frame colliders (placed.ts), a LIVE car CollisionRect the route mutates in place per frame (the host step re-reads rects, so the rising car carries the player — pinned in physics.test.ts), E rides/calls via the interact frame (which now reads keys through embodied.actionDown — the route-local createKeyState is gone, viewport.test.ts guard green again), navGrid stamps the shaft as a link and holds interactable doors OPEN for pathing. P4: build.test.ts, elevators.test.ts, navGrid.test.ts, physics.test.ts. REQ-0652 (2026-06-12, USER: "it works only in the test route. next add the method to the compiled game loader"): THE COMPILED GAME RIDES — the ELEVATORS lump (MAP_LUMP 18; compile/worldElevators.ts derives per-shaft car/module footprint + stops from the SAME GAME_BUILD.elevators source, constructor.zig decodeElevators reads it) replaces the static parked-car delivery; world_loader.zig appends one LIVE car rect per shaft after the baked colliders (car_rect_start/car_count keep cars↔shafts aligned), stepElevators re-aims top/floor in place before each physics step (host groundAt + downhill snap carry the rider), one live car node per shaft renders (the dynamic-prop pattern), and stepInteract grew the board/call E branch and runs with either lump present. Occlusion PARITY (USER report): placedPieceCameraOccluders emits the elevator frame and PlayRoute counts kind elevator as occluding, matching the compiled camera whose set is the COLLIDERS lump. rjit game verify now owns cart/hmsc-int/compile suites (SUITE_ROOTS, cli/commands/game.ts — five suites existed but never ran); 87/87 green. Proof: rjit game shot GREEN with "[loader] elevator layer: 2 live car(s) across 2 shaft(s)". PARITY-0611 (2026-06-11, USER ASK req_0654 hidden-wall doorway + req_0655 \'all green is useless when it isn\'t true end to end\'): the compiled bake baked door/window walls SOLID while colliders shipped the opening — worldGeometry carried PRIVATE wall/stairs/elevator decomposition copies blind to piece.edit. Fix: pure shape math extracted to editors/build/pieceShapes.ts (React-free, BUILD_UI moved with it, pieceMeshes/buildUi re-export) and the bake lowers pieceVisualShapes rows directly (VisualBox.slot interns skin shader/decals; translucent looks ship flat alpha materials); the dark edit placeholder became the CLOSED DOOR PANEL drawn only for interaction edits and sized by PLACED_TUNING portal opening+panel height (render IS the collider) — arch genuinely open, halfHeight no floating box, garage panel fills its 2.6m opening. Hand-mirrored copies deleted (MATERIAL_COLOR/MATERIAL_ALPHA/slab consts/pushStairs/pushElevatorStorey/pushSkinnedWallOrPlate). P4: compile/worldParity.test.ts — 0.1m-grid sample-occupancy parity editor↔bake for EVERY WallEdit row + stairs/elevator baselines + the walk-through⇒see-through invariant; committed RED first (4feaa90bb), green after. DOORS-0611 (same reqs): the compiled two-state door — compile/worldDoors.ts derives one record per interactable wall cutout from the SAME door-marked pieceVisualShapes leaf (forced closed; reach from the edit contract; startOpen carries authored doorOpen) into the DOORS lump (MAP_LUMP 19, constructor.zig decodeDoors); static halves ship WITHOUT the panel (worldGeometry skips door-marked boxes; worldColliders bakes placed.colliders liveDoorPanels — new opt, editor default unchanged); world_loader.zig appends one live rect per door after the car rects (door_rect_start/door_count), renders one live yawed panel node, and stepInteract 2a-doors prompts E — open/close within the record reach, the toggle flipping blocksPlayer in place + dropping/returning the node (instant, pieceDoorSet parity). Parity suite compares the BOOT UNION (static rows + closed leaves). P4: compile/worldDoors.test.ts (6). Proof: rjit game shot GREEN — \'[loader] door layer: 2 live door(s)\' on the user\'s coastal-town map. REQ_0663 (user playtest: \'i can open them. i cant walk through the open door\'): de-flagging blocksPlayer is NOT open — physics.zig collideSolidRects side-pushes any rect too tall to step onto EVEN when non-solid (the walkable-platform law), so the toggle now PARKS the rect 1e9m out of the world while open and restores the exact bounds on close (startOpen doors boot parked the same way; doorHalfExtents is the one bounds source). Deferred by name: travel animation (openSeconds already rides the contract), auto-sliding storefront glass (a new WallEdit row), door sounds.',
    ...
    },
    {
      ...
      description:
        'WO-2 capture (2026-06-04): the kind registries REWRITTEN fresh (V17-TRIAGE) — tiles (18 kinds, LOCKED road grammar with lane flow as table data), props (54 after the catalog grew), NPCs (4 + faction regard matrix), roles (open axis), landforms (4, fixed-shape constants lifted into LANDFORM_TUNING per P2). One door (index.ts → GAME_KINDS via game/index.ts); P4 behavior tests per family under tools/v8cli (shared game/_testkit.ts); CAPTURE.md records dropped dead fields and surfaced ambiguities. PROPMERGE-0611 (review §13.1): game/kinds/props.ts is THE ONE prop table — the legacy twin world/propKinds.ts (709-line value-identical duplicate with SPLIT consumers: editor palette/PropertiesPanel/placements/host physics/traffic/render3d props/compile read it while the game door read the capture — the live divergence hazard) is RETIRED, every consumer repointed, the PropKind union moved INTO the registry (design.ts re-exports it), and req_0623\'s dumpsterBodyMeters one-definition block moved with it. New prop kinds: game/kinds/props.ts ONLY; tools/prop-scale reads the unified table. PROPBATCH-0611 (req_0633/0634/0635): ~100 kinds after the variety drop (grass, jagged rocks, tree size variants, radio tower, gas pump, vending machine, store shelf, sign family, media set, the junkyard set) at real scale × 1.15; a new kind is ONE data recipe in game/kinds/propModels.ts (compiled-loader shape vocabulary: box/cyl8/cyl16/sphere) rendered by /test\'s render3d/props/DataProp.tsx (via TexturedParts — click-to-pick texturable) AND lowered by compile/worldGeometry.ts propParts default — the paths agree by construction. partId parts are flat image targets (req_0635): partTextures[partId] renders live in /test and interns as the part\'s material in the bake (albumCover "cover", poster/signs "face", vendingMachine "front"). New dynamics kinds (tire/steelDrum/propaneTank/jerryCan/brick) + containers (vendingMachine/lockerSet/shippingContainer/crate/barrel/storeShelf) ride lumps 16/17 — zero Zig changes. PROPSHELF-0611: PROP_CATEGORIES shelves every kind into categories (total disjoint partition, suite-enforced); IsoAuthor\'s prop tab picks a shelf first instead of a ~100-button wall. PROPVENUE-0611 (req_0640): +20 kinds on two new shelves — park (sittable plaza fountain, drinking fountain, lounge chair, swing set, sand castle, picket fence, apple tree + kickable apple dynamics prop) and shops (arcade cabinet/menu board/soda machine with image-target panels, slot machine, clothing rack, display case, liquor shelf, beer cases, diner booth, order counter, OPEN sign, dispensary green cross). Deferred by name in registry comments: swing pendulum physics (needs constrained bodies) and apple DROP spawning + throw/eat (item-system slice). GRASSTILE-0611 (req_0642): grass + grassDry are real paintable surface TILES, appended at the END of both tile tables (game/kinds/tiles.ts AND the live editor twin world/tileKinds.ts — append-only index contract) + the design.ts union; quiet underfoot, thin concealment, bad for cars. SCATTERBRUSH-0611 (req_0642): game/kinds/scatter.ts — P2 weighted prop mixes (meadow/forest/rocky) + a pure deterministic per-tile roll; armed from the OBJECTS tab\'s SCATTER BRUSHES group, the place stroke rolls over the brush footprint and stamps ordinary cat-prop placements (repaint never double-fills; occupied tiles skipped); scatter.test.ts 4 P4 cases. Iso CatalogRail tab+shelf are route twigs (req_0643).',
      dependsOn: ['game/_testkit.ts', 'game/index.ts'],
      status: 'live',
    },
    {
