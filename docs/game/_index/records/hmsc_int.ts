...
export const hmsc_int: DocIndex = {
  ...
  interfaces: [
    ...
    },
    {
      ...
      description:
        'CUTOUTAPP-0605 (2026-06-05): cart/cutout\'s APP EXPERIENCE remade as the /cutout route in the one shell — the full-canvas, layer-stack, smart-select image/texture editor for painting SKINS/TEXTURES (the user\'s explicit ask; the earlier head-part-painting landing in /characters was ruled NOT it). Consumes editors/paint whole (PaintEditor — engine never forked); CAPTURE.md is the APP-surface deletion contract (17 rows; the engine\'s 34 are paint\'s own). stream.ts = the V20 \'cutout\' concern: the LIBRARY of saved PaintDocuments (upsert by id, re-openable) + extracted CutoutAssets (source-res binary RLE mask + overlayRes preview cells + pixels + srcPath + source docId); saved/extracted/removed events, unknown kinds pass through; lives route-side until the compile consumes painted assets (graduates behind a game/ door then — the log file never moves). extraction.ts = pure bookkeeping: extractCutout refuses empty selections, mask→asset→mask pinned exact, cutoutToDocument reopens a stored cutout as a one-layer document whose smart base IS the mask (the composability law). sources.ts = the hosting editor\'s half of the engine hand-off: magick identify + grayscale load (dims/srcPath/gray as data). CutoutRoute.tsx = header (name · save · extract · blank canvas W×H + presets · image path/load · status), library rail (PaintQuad cells-mode swatches, open/remove), the full painter remounted per working target, file-drop loads images, smart select auto-arms only with an image source, painter hotkeys ON (the host suppresses key triggers while a TextInput is focused). Session history: /cutout on the cutout channel — strokes/layer-ops are the painter\'s labeled notes; saves/extractions/removals are COMMIT-grade (content event + marker + snapshot). cutout.test.ts 7 P4 cases GREEN (editors suite root). Surfaced, not guessed: file exports (PNG/pixel-icon/.sqi) deliberately absent pending the user\'s export ruling — the stream asset carries everything a file exporter needs; extraction chose commit-grade (the asset must persist); no mid-stroke document autosave (a stroke is a note, not a full-doc event).',
