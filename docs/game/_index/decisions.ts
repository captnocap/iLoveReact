...
export const DECISIONS: Decision[] = [
  ...
  {
    id: 'V28', name: 'Platform/mod split: Roblox/GMod tier — hmsc is the FIRST MOD; games are script+data packages on ONE player binary (PLATMOD-0607)', status: 'ruled',
    ruling: 'Three lifetimes, three layers: framework/+runtime/ = the platform tier and owns the versioned GAME-AGNOSTIC map format (entity lump is opaque keyvalues the mod binds — the platform never knows what a paramedic is); the world systems (pathing, perception, physics, figures, vehicles, materials) are the base-game/SDK tier and hmsc-int is the platform\'s editor (its Hammer/Studio); hmsc is the FIRST MOD, owning entity semantics, the GameState schema, and the changelevel persistence contract. A game is a SCRIPT+DATA PACKAGE (bundle.js + mapfiles + assets + manifest) loaded at runtime by ONE shipped player binary — never a compiled binary per game; the dev host already is that client (runtime cart loading, hot-swap, tabs), only the packaging shape is missing. V15 AMENDED: the canonical compiled output of hmsc-int is the GAME PACKAGE; a fused standalone binary (rjit ship) remains a distribution option that bakes the package in. Boundary law: hmsc stays the driving game — extract boundaries as it pulls on them, NEVER speculatively; every line passes the second-mod test (could a different game be built from editor+platform without touching framework/ or forking hmsc). @reactjit/* and the map format become versioned public contracts, stabilized by use, not frozen up front.',
    detail: 'User, verbatim: "it would be silly to only retain it in a means for a one off game, and effectively can get away with still building the same game, but its just a \'mod\' on the underlying game engine we are making"; tier: "the ability to make a game is like that of roblox/gmod esque". The HL→CS lesson: Half-Life never designed for CS; it kept the game-DLL boundary honest and shipped the tools, and CS happened to it.',
    keywords: ['platform', 'mod', 'roblox', 'gmod', 'engine', 'game package', 'player binary', 'one binary', 'script and data', 'first mod', 'second mod test', 'map format ownership', 'entity lump', 'opaque entities', 'hammer', 'studio', 'sdk tier', 'base game', 'ship', 'packaging', 'distribution', 'changelevel contract', 'counter-strike', 'half-life', 'bsp lesson'],
    cites: ['cart/hmsc-int/', 'tools/rjit', 'runtime/workspace/rle.ts'],
  },
  {
