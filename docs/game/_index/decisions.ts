...
export const DECISIONS: Decision[] = [
  ...
  },
  {
    ...
    ruling: 'Three lifetimes, three layers: framework/+runtime/ = the platform tier — a STATELESS ZIG ENGINE whose core capabilities (camera, movement, physics, rendering of pieces/textures/models/map, behaviors incl. NPC AI + the 45-tick system) ALREADY EXIST in Zig and are stateless BY DESIGN, taking DATA and running it (user: "all of the capability already exists in zig, it just is \'stateless\' by design"); it owns the versioned GAME-AGNOSTIC RLE data format (V29) and never knows what a map is FOR. The world systems (pathing, perception, physics, figures, vehicles, materials) are the base-game/SDK tier and hmsc-int is the platform\'s editor (its Hammer/Studio). hmsc is the FIRST MOD — entity semantics, the GameState schema, and the changelevel contract, all expressed AS DATA. A GAME IS DATA, never per-game code (user: "an entire game can be rle\'d as long as the core capabilities exist in the engine itself"): shape = { buildings[], textures[], map[], models[], data:[[[…]]] } — an asset vocabulary plus an RLE tape composing them BY REFERENCE (piece→shape→position→face-materials), and that tape IS the state fed to the stateless capabilities. The Compile button does THREE bakes each → RLE: (1) game logic, (2) game map, (3) custom items/skins; the loader "takes in all the data, constructs the game from it". Pipeline: "ts/tsx → encoded rle shape → loaded into zig loader → play". TWO PATHS: /test route = the DYNAMIC dev env (live, "always rencoding", JS-hosted, correct by design); SHIP path = baked RLE + Zig loader, NO JS (end goal: "dropping off the javascript"). V15 AMENDED: the canonical compiled output of hmsc-int is the BAKED RLE GAME DATA; a fused standalone binary (rjit ship) bakes the data in. NO per-game script seam ANYWHERE incl. behavior — logic is engine capability PARAMETERIZED BY DATA; a missing capability EXTENDS THE ENGINE (test-route → finalize-into-built-engine), never the game file. Second-mod test: a different game = DIFFERENT DATA, no framework/ change and no hmsc fork.',
    detail: 'User, verbatim: "it would be silly to only retain it in a means for a one off game … but its just a \'mod\' on the underlying game engine we are making"; tier: "roblox/gmod esque"; the HL→CS lesson (kept the game-DLL boundary honest, shipped the tools). CORRECTION 2026-06-08 (req_0287): the original draft\'s "SCRIPT+DATA PACKAGE (bundle.js …) loaded by ONE player binary; the dev host already IS that client (persistent V8 host)" was a WORKER INTERPRETATION the user never made and it contradicts "dropping off the javascript" — the real model is the stateless-Zig-engine + RLE-data + Zig-loader above. PLATMOD slice 1 (commit 92c703fa2, rjit-player loading bundle.js via V8) is the TESTING-ENV loader the user called wrong (req_0254), NOT the shipped Zig loader — history, not the ship path; its RLE codec/lump container/content addressing survive, the V8 premise does not. V29 (RLE format) and V30 (frozen world) are consistent and unchanged.',
    keywords: ['platform', 'mod', 'roblox', 'gmod', 'engine', 'stateless engine', 'stateless zig', 'game is data', 'rle data', 'rle tape', 'zig loader', 'loader', 'three streams', 'three bakes', 'compile button', 'logic to rle', 'map to rle', 'items skins to rle', 'asset vocabulary', 'by reference', 'first mod', 'second mod test', 'data format ownership', 'opaque data', 'hammer', 'studio', 'sdk tier', 'base game', 'ship', 'dropping off javascript', 'no js', 'no per-game script', 'extend the engine', 'test route dynamic', 'ts tsx to rle to zig to play', 'counter-strike', 'half-life', 'slice 1 wrong loader', '92c703fa2'],
