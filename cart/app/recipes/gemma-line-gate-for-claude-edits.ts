...
export const recipe: RecipeDocument = {
  ...
  instructions:
    "Two-round benchmark for the failure mode where Claude is asked to edit a file from a stale line-number manifest. Round 1 hands Claude the file and manifest with a soft, non-leading prompt. Round 2 puts a local Gemma-4-E2B model — loaded through framework/assistant/local_ai_runtime.zig and kept linker-alive by framework/assistant/llama_exports.zig — in front of Claude as a TRUE/FALSE line-checker, and prepends its verdicts as a preamble before Claude is allowed to act. Both models ride the same useAssistant hook with different backends.",
  sections: [
    {
      kind: "paragraph",
      text:
    ...
    },
    {
      kind: "paragraph",
      text:
        "Gemma runs through this repo's own llama runtime — framework/assistant/llama_exports.zig keeps the libllama_ffi.so symbols alive against the linker, and framework/assistant/local_ai_runtime.zig consumes them as a Session API. Both Claude and Gemma are reached through the same useAssistant hook — different `backend` values pick the underlying worker. No HTTP server, no LM Studio, no extra process.",
    },
    {
      kind: "bullet-list",
      ...
      items: [
        ...
        "Round 1: a useAssistant({ backend: 'claude_code' }) cart with a soft prompt — \"glance at this and tell me what you think.\" No mention of line numbers, no hint that the manifest is stale.",
        "Round 2: a TSX cart that mounts useAssistant({ backend: 'local_ai', modelPath: gemmaPath }), walks the manifest, and emits a TRUE/FALSE preamble. The preamble is then concatenated with the same prompt and sent to a Claude useAssistant.",
      ],
    },
    {
      kind: "code-block",
...
experiments/manifest_check/manifest.md`,
    },
    {
      kind: "paragraph",
      text:
        "The same local_ai_runtime.zig that powers useEmbed's reranker drives generation here — it's already a Session with submit/poll. We just open it with a chat model instead of an embedding model, behind useAssistant({ backend: 'local_ai' }).",
    },
    {
      kind: "paragraph",
...
function Round1({ manifestText, fileText }: { manifestText: string; fileText: string }) {
  ...
  return <RenderEvents events={events} />;
    ...
    },
    {
      ...
      text:
        "Mount one useAssistant against the local Gemma model, then drive the manifest one claim at a time off the events stream. Each request ships the full file with line numbers prefixed and asks exactly one question: does line N say what the manifest says it says? The sampler is forced to one of two tokens.",
    },
    {
      kind: "code-block",
      language: "tsx",
...
function ManifestGate({ file, claims }: { file: string; claims: { line: number; claim: string }[] }) {
  ...
  return verdicts.map((v, i) => ({ line: claims[i].line, verdict: v, claim: claims[i].claim }));
    ...
    },
    {
      ...
      items: [
        "One claim per ask(), not one batched call. Stuffing all fourteen claims into one prompt collapses Gemma's accuracy — it starts hedging. One claim in, one token out, repeat.",
        "maxTokens: 4 in the opts. The sampler has no room to be creative, and the constrained shape makes parsing trivial: anything that doesn't start with TRUE or FALSE is treated as UNCLEAR and re-asked once with the line content quoted back at it.",
      ],
    },
    {
      kind: "paragraph",
      text:
        "Gemma-4's context window is >200k tokens. Set nCtx: 262144 and forget about chunking entirely — even a 10k-line file fits in one prompt with room left over for every claim's preamble, the tokenized line-number prefix, and a system message. The KV cache only fills the part of the window you actually use, so the cost is the file's real token count, not the configured ceiling. Practically: you can keep the same warm worker across files, prepend a directory of source on first use, and let the gate answer claims about any line in any file the cart has shown it without reloading.",
    },
    {
      kind: "paragraph",
...
=== END PREAMBLE ===`,
    ...
    },
    {
      ...
      items: [
        ...
        "Streaming gate. The useAssistant events stream emits assistant_message chunks as the model decodes — for very large files you can render preliminary verdicts and start prompting Claude before the gate finishes.",
        "Wider models. Swap the Gemma modelPath for a Qwen3.6-27B-Q4_K_M.gguf — same hook, different opts: ~10× slower, marginal accuracy gain. Worth it only when claims aren't pure substring checks.",
        "Other claim shapes. useAssistant doesn't know about line numbers. Swap the prompt for \"is foo defined in this file?\" or \"does this function have early returns?\" and the same TRUE/FALSE preamble works.",
      ],
    },
  ],
};
