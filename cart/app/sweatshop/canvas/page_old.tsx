...
import { Box, Col, Row, Text, Pressable, Canvas, Icon, ScrollView } from '@reactjit/runtime/primitives';
            ...
            {stagedActive ? (
              <Box style={{
                ...
                <Box style={{
                  ...
                  <Icon icon={Sparkles} size={11} strokeWidth={2} color="theme:accent" />
              ...
              <Box style={{
                ...
                <Box style={{
                  ...
                  <Icon icon={Move} size={11} strokeWidth={2} color="theme:accent" />
                ...
                {[...stashed, ...Array.from(live.hidden).filter((id) => !stashed.includes(id))].map((id) => {
                  ...
                  return (
                    <Pressable key={id} onPress={stagedActive ? () => {} : () => {
                      ...
                      <Icon icon={def.icon} size={14} strokeWidth={2} color="theme:inkDim" />
                    </Pressable>
                  );
                })}
              </Box>
    ...
    <Col style={{
      ...
      <Pressable onMouseDown={editMode ? onDragHandle : undefined}>
        <Row style={{
          ...
          <Icon icon={def.icon} size={11} strokeWidth={2} color="theme:inkDim" />
          <Text size={9} color="theme:inkDim" bold>{def.label.toUpperCase()}</Text>
          <Box style={{ flexGrow: 1 }} />
          {editMode ? (
            <Pressable onPress={onClose}>
              <Icon icon={X} size={11} strokeWidth={2} color="theme:inkDim" />
            </Pressable>
          ) : null}
        </Row>
      </Pressable>
    ...
    <Row style={{ flexGrow: 1, gap: 2, padding: 2, alignItems: 'center' }}>
      {tabs.map((t, i) => (
        <Pressable key={t.label} style={{
          ...
          <Icon icon={t.icon} size={11} strokeWidth={2} color={i === 0 ? 'theme:accent' : 'theme:inkDim'} />
  ...
  return (
    <Row style={{ flexGrow: 1, padding: 2, gap: 2, alignItems: 'center' }}>
      {toggles.map((p) => {
        ...
        return (
          <Pressable key={p.id} onPress={() => togglePanel(p.id)} style={{
            ...
            <Icon icon={p.icon} size={12} strokeWidth={2} color={on ? 'theme:accent' : 'theme:inkDim'} />
          </Pressable>
        );
      })}
      <Box style={{ flexGrow: 1 }} />
      <Pressable onPress={editToggle} style={{
        ...
        <Icon icon={Move} size={11} strokeWidth={2} color={editMode ? 'theme:accent' : 'theme:inkDim'} />
      ...
      {dragSlotRef.current != null ? (() => {
        ...
        return (
          <Box style={{
            ...
            <Icon icon={atom.icon} size={Math.max(10, Math.floor(slotPx * 0.45))} strokeWidth={2} color="theme:accent" />
          </Box>
        );
      })() : null}
    </Box>
...
function MinimapStub() {
  return (
    <Box style={{ flexGrow: 1, alignItems: 'center', justifyContent: 'center' }}>
      <Icon icon={MapIcon} size={20} strokeWidth={2} color="theme:inkDim" />
    </Box>
  );
}
