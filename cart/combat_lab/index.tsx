...
function makeBot(id: string, patrol: V3[]): Bot {
  ...
  return {
    ...
    fleeUntil: 0, lastHitAt: -9, lastHitBone: null,
...
export default function CombatLab() {
  ...
  // skeleton (walk gait, kneel+crouch blend for the player) and places it at
  // pos/heading; a ragdolled body's particles ARE the bones, already in world.
  const solveBots = (bot: Bot): Record<BoneId, SkeletonBone> => {
    ...
    const local = buildSkeleton(def.shape, bot.moving ? 'walk' : 'stand', bot.gait % 1);
  ...
  const playerFire = () => {
    ...
    if (pick && pick.hit.t < coverT) {
      ...
      if (!npcKindDefinition(bot.kind).canFight) {
        bot.mode = bot.hp <= 0 ? bot.mode : 'flee';
        bot.fleeUntil = s.t + 4;
      ...
      if (!ui.paused) {
        ...
        for (const bot of s.bots) {
          ...
          if (bot.moving) {
            ...
            if (bot.mode !== 'engage') bot.heading = headingOf(vx, vz);
  ...
  // hit. Merged with hitboxes/tracers/sparks into ONE keyed list rendered
  // last (the reconciler's variable-array rule).
  type Fx =
