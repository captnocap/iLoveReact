// verifyHarness.ts — the command-script replay harness (V19).
//
// The actual hmsc-int Compile button / compiled route uses `rjit game bake` and
// writes a platform game-file for the no-V8 Zig loader. This file is not that
// compiler. `rjit game verify` bundles this harness, boots it under tools/v8cli
// with a verify script, and reads the verdict. The V19 contract is:
//
//   bundle harness -> boot headless -> replay a command sequence -> verdict
//
...
//
// MILESTONE-0 history: this began as the "game compile" skeleton. That public
// name is retired so nobody mistakes it for the data bake; the replay surface
// remains useful for command vocabulary tests.
...
function main(): number {
  ...
  if (!scriptPath) {
    console.error('usage: v8cli zig-out/game/hmsc-verify-harness.js <verify-script.cmds>');
    return 2;
  }
