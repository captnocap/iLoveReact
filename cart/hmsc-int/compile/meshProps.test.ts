// meshProps.test.ts — the MESH_PROPS lump v4 (req_1544): the cooked-prop paint
...
function meshRow(key: string, png?: Uint8Array): any {
  return {
    key, source: `t:${key}`,
...
// Mirror of constructor.zig decodeMeshProps (v4) — the contract under test.
function decode(bytes: Uint8Array): { version: number; meshes: { key: string; pngLen: number; png: Uint8Array | null }[]; instances: number } {
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const version = view.getUint32(0, true);
  const meshCount = view.getUint32(4, true);
  const instanceCount = view.getUint32(8, true);
  let at = 12;
  const meshes: { key: string; pngLen: number; png: Uint8Array | null }[] = [];
  for (let m = 0; m < meshCount; m += 1) {
    ...
    at += pngLen;
    meshes.push({ key, pngLen, png });
  }
  return { version, meshes, instances: instanceCount };
}

test('MESH_PROPS v4: a cooked prop ships its paint as PNG bytes, round-trips byte-exact (req_1544)', () => {
  const png = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 1, 2, 3, 4, 250, 251]); // PNG sig + payload
  const sink: ImportedMeshPropSink = {
    meshes: [meshRow('painted', png), meshRow('bare')] as any,
    instances: [{ mesh: 0, x: 1, y: 2, z: 3, yawDegrees: 90 }, { mesh: 1, x: 0, y: 0, z: 0, yawDegrees: 0 }],
  };
  const out = decode(encodeMeshProps(sink));
  assertEqual(out.version, 4, 'lump is v4');
  assertEqual(MESH_PROPS_LUMP_VERSION, 4, 'the exported version constant is 4');
  assertEqual(out.instances, 2, 'both instances encoded');
