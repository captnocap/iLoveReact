...
import { foliageCells } from '../render3d/grassPopulation';
import { buildPalmTrunkInstances, palmCells } from '../render3d/palmPopulation';
...
/** Palm groves: each spawned palm = a TRUNK mesh (baked as a distinct object, the
 *  user's ruling) + a crown of FROND cards (recipe-ized req_1861 — the fronds ride the
 *  FLORA lump as one cell per palm, regenerated by the loader's foliage.zig palmCrown,
...
function pushPalms(b: Build, state: GameState, floors: readonly ChunkFloor[]): void {
  const trunks = buildPalmTrunkInstances(worldWithFloorLandforms(state, floors));
  console.warn(`[bake] palm: ${trunks.count} trunk(s) baked; fronds ride the FLORA recipe`);
  pushInstanceField(b, trunks, INSTANCE_SHAPE_PALMTRUNK);
}
...
export function encodeFlora(state: GameState, floors: readonly ChunkFloor[]): Uint8Array {
  ...
  // grass + bush + flowers (foliageCells) PLUS palm crowns (palmCells) — all regenerated by
  // the loader's foliage.zig, so none of them enumerate rows into the V8 bake heap (req_1861).
  const cells = [...foliageCells(world), ...palmCells(world)];
