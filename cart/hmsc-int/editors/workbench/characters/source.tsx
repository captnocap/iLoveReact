...
// (WBCHAR-0606, the pattern-setter). Parity rows land per WBCHAR.CAPTURE.md:
// the roster (A5-A7), the generated panel (B/C/E/F/G/H rows), hero actions
// (A4, A6, B2, B3), the FIGURE/PART/SCULPT/PAINT lens set (C2), all reading
// and writing the ONE character store — the same doors the stage uses.
//
// No layout in here: panel() returns a SPEC the shared field renderer draws
// (the workbench protocol; settings speak it from the tunables registry,
// characters speak it from the draft's setters).
...
export function charactersSource(store?: CharacterStore): WorkbenchSource<CharacterStore> {
  ...
  return {
    ...
    // commit 3 mounts CharacterStage here (3D grab / sculpt canvas / paint)
    stage: () => null,
  };
}
