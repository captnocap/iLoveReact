...
export function HotkeysPanel(props: { scope: EditorScope | EditorScope[]; onClose: () => void }) {
  const rerender = useRerender();
  // Mid-capture key is `${scope}:${action}` so a row is unambiguous when more than
  // one scope is shown together (e.g. 'studio' + 'studio-paint' in paint mode).
  const [capturing, setCapturing] = useState<string | null>(null);
  const [conflict, setConflict] = useState<string | null>(null);
  const scopes = Array.isArray(props.scope) ? props.scope : [props.scope];
  const rows = scopes.flatMap((s) => bindingsForScope(s));
  ...
  useEffect(() => {
    if (!capturing) return;
    const sep = capturing.indexOf(':');
    const capScope = capturing.slice(0, sep) as EditorScope;
    const capAction = capturing.slice(sep + 1);
    setKeyCapture(true);
    const off = busOn('__keydown', (e: any) => {
      const base = String(e?.key ?? '').toLowerCase();
      if (!base) return;
      if (base === 'escape') { setCapturing(null); return; }
      const r = rebind(capScope, capAction, [chordOf(e)]);
  ...
  return (
    <Box style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', backgroundColor: '#03060caa', zIndex: Z.modal }}>
      <Col style={{ gap: 8, paddingLeft: 16, paddingRight: 16, paddingTop: 14, paddingBottom: 14, borderRadius: 10, backgroundColor: '#0e1726f5', borderWidth: 1, borderColor: '#2c4a6a', minWidth: 380 }}>
        ...
        {rows.map((b) => {
          const overridden = isOverridden(b.scope, b.action);
          const slot = `${b.scope}:${b.action}`;
          const mid = capturing === slot;
          return (
            <Row key={slot} style={{ gap: 10, alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <Pressable onPress={() => { setConflict(null); setCapturing(mid ? null : slot); }} tooltip="Press a key to rebind this action" style={{ paddingLeft: 8, paddingRight: 8, paddingTop: 3, paddingBottom: 3, borderRadius: 5, backgroundColor: mid ? '#3a2f5e' : '#13233aee', borderWidth: 1, borderColor: mid ? '#9b7fd6' : '#2c4a6a' }}>
                <Text fontSize={10} color={mid ? '#e0d4ff' : T.dim}>{mid ? 'cancel' : 'rebind'}</Text>
              </Pressable>
              <Pressable onPress={overridden ? () => { resetBind(b.scope, b.action); setConflict(null); rerender(); } : undefined} tooltip="Reset to the default key" style={{ paddingLeft: 8, paddingRight: 8, paddingTop: 3, paddingBottom: 3, borderRadius: 5, opacity: overridden ? 1 : 0.35, backgroundColor: '#13233aee', borderWidth: 1, borderColor: '#2c4a6a' }}>
