...
const SEALED_HERE = [
  ...
  'GAME_ANIMATION', 'GAME_VEHICLE', 'GAME_TELEMETRY',
];
...
test('live doors are live; capture-pending doors say so honestly', () => {
  const live = [...SEALED_HERE, 'GAME_KINDS', 'GAME_CHANCE', 'GAME_PERCEPTION', 'GAME_ITEMS', 'GAME_CHROME', 'GAME_CUTSCENE', 'GAME_ACTIVITIES'];
...
test('the live doors carry their interface, not a grab-bag', () => {
  ...
  assertEqual(typeof door.GAME_TELEMETRY.readFrame, 'function', 'GAME_TELEMETRY.readFrame');
  assertEqual(typeof door.GAME_TELEMETRY.copyDiagnostics, 'function', 'GAME_TELEMETRY.copyDiagnostics');
});
