...
export default function ColorLibraryPanel(props: {
  ...
  // ── Map + chroma drag (the ColorField pattern) ─────────────────────────────
  ...
  const pickMap = (p: any) => {
    ...
    props.onSetCurrent({ l: 1 - y, c: currentRef.current.c, h: x * 360 });
  };
  const pickChroma = (p: any) => {
    if (!chromaRect) return;
    const x = clamp01((Number(p?.x) - chromaRect.x) / Math.max(1, chromaRect.width));
    props.onSetCurrent({ ...currentRef.current, c: x * MAX_CHROMA });
  };
  const start = (t: 'map' | 'chroma', p: any) => { dragRef.current = t; if (t === 'map') pickMap(p); else pickChroma(p); };
  const move = (t: 'map' | 'chroma', p: any) => { if (dragRef.current !== t) return; if (t === 'map') pickMap(p); else pickChroma(p); };
  const end = () => { dragRef.current = null; };

  // Marker positions from the current color.
  ...
  return (
    <Col style={{ gap: 8 }}>
      ...
      {/* MAP — hue across, lightness down to BLACK; chroma strip below. */}
      <Col style={{ gap: 4 }}>
        <Pressable
        ...
        >
          <Box onLayout={(r: any) => setMapRect(r)}
            ...
            {mapRect ? (
              <Effect shader={OKLCH_MAP_SHADER} data={[current.c, 0, 0, 0]}
        ...
        >
          <Box onLayout={(r: any) => setChromaRect(r)}
            ...
            {chromaRect ? (
              <Effect shader={OKLCH_MAP_SHADER} data={[current.c, 1, clamp01(current.l), hue]}
