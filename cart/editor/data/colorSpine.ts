...
/** Deletable STARTER sets seeded into the color library exactly once — on the
 *  first boot where the save file has no `sets` field. From then on the file
 *  is the only truth: rename, edit, delete, all real (the hardcoded
 *  SPINE_LIBRARY facade this replaces never persisted anything). The old
 *  PIGMENTS reference dabs live on as the "Pigments" starter set. */
export function starterPaletteSets(): SpinePaletteSet[] {
  return [
    { name: 'Pigments', colors: MIX_PIGMENT_DEFS.map((p) => ({ l: p.l, c: p.c, h: p.h })) },
 ...
 *  matched swatch gets ringed and the set gets a "has yours" tag. Pure
 *  derivation over the REAL persisted sets. */
export function libraryRows(current: OklchColor, sets: readonly SpinePaletteSet[]): LibraryRow[] {
  return sets.map((set) => {
    let matchedIndex = -1;
    let best = Infinity;
