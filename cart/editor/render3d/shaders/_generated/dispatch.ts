...
fn surface_brick(sp: vec2f, seed: f32) -> SurfaceSample {
  let grid = vec2f(sp.x / mat_param(1, 1.0), sp.y / mat_param(2, 1.0));
...
fn surface_corrugated(sp: vec2f, seed: f32) -> SurfaceSample {
  let phase = sp.x * 6.28318531 / mat_param(0, 1.0);
  let corr = sin(phase);
  let ridge = corr * 0.5 + 0.5;
  let cell = vec2f(floor(sp.x / mat_param(0, 1.0)), floor(sp.y));
  let height = corr * mat_param(1, 0.012) * 0.5;
  let tone = rand(cell + vec2f(seed, seed * 2.0));
