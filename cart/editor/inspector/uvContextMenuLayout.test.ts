...
test('texture menu first frame budgets every action including all UV guide exports', () => {
  const height = uvContextMenuHeight('texture', { group: null, height: 0 });
  assert(UV_CONTEXT_MENU_TUNING.expandedRows.texture === 11, 'Texture Atlas row estimate lost an action');
  assert(height === 330 + 11 * 26, `texture fallback height drifted to ${height}`);
});
