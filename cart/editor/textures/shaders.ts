...
function fillSpec(b: FillBoard, m: FillMaterial): ShaderSpec {
  const registryMaterial = MATERIALS.find((candidate) => candidate.boardIndex === b.board && candidate.slug === m.slug);
  const materialId = registryMaterial?.materialId ?? -1;
  ...
  const fn = registryMaterial?.fn ?? fnForMaterialRow(materialId, b.board);
...
const FILL_SPECS: ShaderSpec[] = FILL_BOARDS.flatMap((b) => b.materials.map((m) => fillSpec(b, m)));
...
export function shaderGroups(): { group: string; specs: ShaderSpec[] }[] {
  ...
  // Reusable imported art is the primary paint library. Procedural recipes
  // remain available after it for the few looks that genuinely need a shader.
  for (const spec of [...IMPORTED_SPECS, ...EDITOR_SHADERS]) {
    const g = out.find((x) => x.group === spec.group);
    if (g) g.specs.push(spec); else out.push({ group: spec.group, specs: [spec] });
  }
  const rank = (group: string) => {
    if (group === 'Imported') return -1;
