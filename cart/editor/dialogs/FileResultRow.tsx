...
export default function FileResultRow(props: {
  ...
  return (
    <Row onPress={() => props.onSelectFile(props.file.id)}>
      <Icon name={explorerFileIcon(props.file)} size={15} color={accentFor(props.active ? 'primary' : 'textSecondary')} />
      <C.HW_FileResultMain>
        ...
        <C.HW_FileSummary>{`${props.file.kind.toUpperCase()} · modified ${props.file.modifiedLabel}`}</C.HW_FileSummary>
      </C.HW_FileResultMain>
